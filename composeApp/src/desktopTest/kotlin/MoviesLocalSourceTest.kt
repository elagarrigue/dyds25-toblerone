...
class MoviesLocalSourceTest {
    ...
    fun `adding Empty List`(){
        ...
        //assert
        assertEquals(emptyList<Movie>(),localMovies.getCacheMovies())
    }
    @Test
    fun `adding Full List`(){
        //arrange
        val movie1= Movie(1, "m1", "x", "x", "x", "x","x","x",7.0,8.0)
        val movie2= Movie(1, "m2", "x", "x", "x", "x","x","x",7.0,8.0)
        val full=listOf<Movie>(movie1,movie2)
        //act
        localMovies.addAll(full)
        //assert
        assertEquals(full,localMovies.getCacheMovies())
    }
    @Test
    fun `is Empty True`(){
        //arrange
        ...
        localMovies.addAll(empty)
        //assert
        assertEquals(true,localMovies.isEmpty())
    }
    @Test
    fun `is Empty False`(){
        //arrange
        ...
        localMovies.addAll(full)
        //assert
        assertEquals(false,localMovies.isEmpty())
    }
}